...
and checkExp  (ftab : FunTable)
    ...
    *)
    | Filter (farg, a_exp, tp, pos) ->
        let (arr_type, arr_exp_dec) = checkExp ftab vtab a_exp
        let elem_type =
            match arr_type with
              | Array t -> t
        ...
        let (f', f_res_type, f_arg_type) =
            match checkFunArg ftab vtab pos farg with
              | (f', Bool, [a1]) -> (f', Bool, a1)
              | (_, res, [a1]) -> 
                   reportTypeWrong "return type of first argument of filter" Bool res pos
              | (_, res, args) ->
        ...
        if elem_type <> f_arg_type then
          reportTypesDifferent "function-argument and array-element types in filter"
                               f_arg_type elem_type pos
        (arr_type, Filter (f', arr_exp_dec, elem_type, pos))
    ...
    *)
    | Scan (farg, ne, a_exp, tp, pos) ->
        let (e_type  , e_dec  ) = checkExp ftab vtab ne
        let (arr_type, arr_dec) = checkExp ftab vtab a_exp
        let elem_type =
            match arr_type with
              | Array t -> t
